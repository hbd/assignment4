...
public class Decryption {
    ...
    public static void invSubAllBytes(State state, SBox sbox) {
	for (int i = 0; i < state.stateMatrix.length; i++) {
	    for (int j = 0; j < state.stateMatrix[i].length; j++) {
		state.stateMatrix[i][j] = Decryption.subByte(state.stateMatrix[i][j], sbox.invSbox);
	    }
	}
    }
    ...
    // XoRing with expandedKey, using round * 4 to choose start column for
    // expanded key
